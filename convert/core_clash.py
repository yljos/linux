...
from parsers import is_valid_clash_yaml, filter_node_names_clash, process_proxy_config_clash
...
def fetch_yaml_text_clash(url: str, source_name: str, force_refresh: bool, cache_dir: Path, cache_expire: int):
    ...
    if not force_refresh and yaml_cache_file.exists():
        try:
            ...
            logger.warning(f"Failed to read cache attributes, attempting network request: {e}")

    if force_refresh: logger.info(f"[{source_name}] [Clash] [Received u]")
...
def process_yaml_content_clash(yaml_text: str, template_path: Path, up_pref: str, down_pref: str, shared_kw: list, shared_ex_kw: list, clean_node_fn):
    try:
        ...
        if not isinstance(input_data, dict): raise ValueError("[Invalid YAML Format]")
            ...
            for group in raw_groups:
                if "filter" in group:
                    ...
                    try:
                        ...
                        group["proxies"] = existing_proxies + [n for n in matched_names if n not in existing_proxies]
                    except Exception as e:
        ...
        output = yaml.dump(template_data, allow_unicode=True, sort_keys=False, default_flow_style=False, width=4096)
