...
class StatusBar:
    ...
    def _update_cpu_baseline(self):
        ...
        try:
            with open('/proc/stat', 'r') as f:
                ...
                cpu_user, cpu_nice, cpu_system, cpu_idle = int(line[1]), int(line[2]), int(line[3]), int(line[4])
                cpu_iowait, cpu_irq, cpu_softirq = int(line[5]), int(line[6]), int(line[7])
                self.prev_cpu = cpu_user + cpu_nice + cpu_system + cpu_idle + cpu_iowait + cpu_irq + cpu_softirq
    ...
    def update_cpu(self):
        ...
        try:
            with open('/proc/stat', 'r') as f:
                ...
                curr_cpu = cpu_user + cpu_nice + cpu_system + cpu_idle + cpu_iowait + cpu_irq + cpu_softirq
                curr_idle = cpu_idle
    ...
    def update_bluetooth(self):
        ...
        # 检查bluetoothd是否运行
        try:
            subprocess.run(['pgrep', '-x', 'bluetoothd'], 
    ...
    def update_ime(self):
        """更新输入法状态"""
        try:
            result = subprocess.run(['fcitx5-remote'], 
                                  capture_output=True, text=True, timeout=2)
