...
def is_running(name: str) -> bool:
    ...
    try:
        return subprocess.run(['pgrep', '-x', name], stdout=DEVNULL, stderr=DEVNULL).returncode == 0
...
def notify_delayed(delay: int, title: str, message: str) -> None:
    ...
    def _notify():
        try:
            subprocess.run(['notify-send', title, message], stdout=DEVNULL, stderr=DEVNULL)
        except Exception:
            pass
...
def start_process(cmd, cwd=None):
    ...
    try:
        if isinstance(cmd, (list, tuple)):
            subprocess.Popen(cmd, stdout=DEVNULL, stderr=DEVNULL, stdin=DEVNULL, cwd=cwd)
        else:
            # string command -> run via shell (keeps same behavior for sh scripts)
            subprocess.Popen(cmd, shell=True, stdout=DEVNULL, stderr=DEVNULL, stdin=DEVNULL, cwd=cwd)
    except Exception:
        pass


def start_if_not_running(name: str, cmd, notify_delay: int = 0, notify_msg: str = '') -> None:
...
def main():
    ...
    # Start dunst
    start_if_not_running('dunst', ['dunst'], notify_delay=0, notify_msg='启动 dunst 通知守护进程')

    # Start swww-daemon
    start_if_not_running('swww-daemon', ['swww-daemon'], notify_delay=2, notify_msg='启动 swww-daemon 壁纸守护进程')
    ...
    start_if_not_running('firefox', ['firefox'], notify_delay=8, notify_msg='启动 Firefox 浏览器')

    # Start Telegram if not running
    start_if_not_running('Telegram', ['Telegram'], notify_delay=10, notify_msg='启动 Telegram 即时通讯')
