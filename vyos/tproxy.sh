...
echo "正在配置 mihomo TProxy (智能防泄露版)..."
...
# 配合 mihomo 的 sniff 功能，这会完美处理所有 DNS
# nft add rule ip mihomo prerouting iifname "$LAN_IF" meta l4proto { tcp, udp } th dport 53 meta mark set $PROXY_FWMARK
# nft add rule ip mihomo prerouting meta mark $PROXY_FWMARK meta l4proto { tcp, udp } counter tproxy to :$PROXY_PORT accept
nft add rule ip mihomo PREROUTING udp dport 53 redirect to :1053
