...
ap.add_argument(
    "--decryption-pipe",
    type=shlex.split,
    default=DECRYPTION_PIPE,
    help="decryption command (string), reads from stdin and writes "
    'to stdout, default: "{}"'.format(" ".join(DECRYPTION_PIPE)),
)
ap.add_argument(
    "--encryption-pipe",
    type=shlex.split,
    default=ENCRYPTION_PIPE,
    help="encryption command (string), reads from stdin and writes "
    'to stdout, suggested: "{}"'.format(" ".join(ENCRYPTION_PIPE)),
)
ap.add_argument(
    "--client-id", type=str, default="", help="Provider id from registration"
)
ap.add_argument(
    "--client-secret",
    type=str,
...
if args.format == "msasl":
    ...
    print(
        ...
        base64.standard_b64encode(
            build_sasl_string(args.protocol, args.email).encode()
        ).decode(),
    )
