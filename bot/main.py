...
def render_notice(action: str, user_id: str, user_obj=None, reason: str = ""):
    lines = [f"{action} user"]
    if user_obj:
        name, uname = getattr(user_obj, "first_name", ""), getattr(
            user_obj, "username", ""
        )
        if name or uname:
...
async def handle_ban_action(
    ...
    if notify_admin:
        ...
        await context.bot.send_message(
            ADMIN_ID, render_notice(action_str, user_id, user_obj, reason)
        )
...
async def s_command(update: Update, context: CallbackContext):
    ...
    if uid in blocked_users:
        return
    if uid != ADMIN_ID:
        return await handle_ban_action(
            context, uid, "Unauthorized /s command", is_ban=True, notify_admin=True
        )
    ...
    with suppress(httpx.RequestError):
        async with httpx.AsyncClient(follow_redirects=True) as client:
            ...
            return await send_temp_message(
                update, context, "OK" if res.status_code == 200 else "Er"
            )
...
async def toggle_ban(update: Update, context: CallbackContext, is_ban: bool):
    ...
    uid_target = (
        extract_user_id(update.message.reply_to_message.text)
        if update.message.reply_to_message
        else (context.args[0] if context.args else None)
    )
    ...
    if not uid_target:
        return await send_temp_message(
            update,
            context,
            f"Usage: /{'ban' if is_ban else 'unban'} <user_id> or reply",
        )

    if not is_ban and uid_target not in blocked_users:
        return await send_temp_message(
            update, context, f"User {uid_target} was not banned"
        )
    ...
    chat = None
    with suppress(Exception):
        chat = await context.bot.get_chat(int(uid_target))
    await send_temp_message(
        update,
        context,
        render_notice("Banned" if is_ban else "Unbanned", uid_target, chat, reason),
    )


async def ban(update: Update, context: CallbackContext):
    await toggle_ban(update, context, True)


async def unban(update: Update, context: CallbackContext):
    await toggle_ban(update, context, False)
...
async def forward_to_admin(update: Update, context: CallbackContext):
    ...
    if chat_id != ADMIN_ID:
        ...
        if chat_id not in whitelist_users:
            return await handle_ban_action(
                context,
                chat_id,
                "Immediate Ban: Unauthorized message",
                is_ban=True,
                notify_admin=False,
            )
        ...
        if not msg.text:
            return await handle_ban_action(
                context, chat_id, "Non-text message", is_ban=True, notify_admin=False
            )
        if len(msg.text) > MAX_MSG_LEN:
            return await handle_ban_action(
                context, chat_id, "Message too long", is_ban=True, notify_admin=False
            )
        ...
        if msg.text == "Hi":
            return await handle_ban_action(
                context,
                chat_id,
                "Immediate Ban: Sent 'Hi' after verification",
                is_ban=True,
                notify_admin=False,
            )
        ...
        markup = InlineKeyboardMarkup(
            [[InlineKeyboardButton("Ban", callback_data=f"ban_{chat_id}")]]
        )
        with suppress(Exception):
    ...
    if msg.reply_to_message:
        ...
        if text_lower == "/ban":
            return await ban(update, context)
        if text_lower == "/unban":
            return await unban(update, context)
...
async def button_callback(update: Update, context: CallbackContext):
    ...
    await query.answer()
    if str(update.effective_user.id) != ADMIN_ID or not query.data.startswith("ban_"):
        return

    target_id = query.data.split("_")[1]
    await handle_ban_action(
        context, target_id, "Quick ban via button", is_ban=True, notify_admin=True
    )
    with suppress(Exception):
