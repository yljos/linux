...
# --- 3. Behavior Settings ---
UPDATE_INTERVAL_MEDIUM=5 # Memory
UPDATE_INTERVAL_LONG=60  # Time
SEPARATOR="|"
...
print_status_bar() {
	local output="${ICON_ARCH}${ARCH}${SEPARATOR}${TEMP_STATUS}${SEPARATOR}${ICON_CPU}${CPU_STATUS}${SEPARATOR}${ICON_MEM}${MEM_STATUS}"
	output="${output}${SEPARATOR}${NET_STATUS_STR}"
	output="${output}${SEPARATOR}${TIME_STATUS}"
	# Clean up any potential double separators
...
# Traps
trap 'print_status_bar' SIGRTMIN+3
