...
def dedup_and_format_ip_yaml(file_path: Path):
    """
    对单个IP规则YAML文件进行去重、排序和格式化，并安全地覆盖原文件。
    """
    ...
    try:
        ...
        ipv4_list.sort()
        ipv6_list.sort()
        data["payload"] = ipv4_list + ipv6_list + other_items
        ...
        print(f"错误: YAML格式无法解析 - {file_path}", file=sys.stderr)
...
if __name__ == "__main__":
    # 获取当前目录下所有以 -ip.yaml 结尾的文件
    current_dir = Path(".")
    target_files = list(current_dir.glob("*-ip.yaml"))

    if not target_files:
        print("未发现匹配 *-ip.yaml 的文件。")
    else:
        print(f"共发现 {len(target_files)} 个匹配文件。")
        for file in target_files:
            dedup_and_format_ip_yaml(file)
